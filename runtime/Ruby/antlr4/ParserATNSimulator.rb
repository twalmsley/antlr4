...
class ParserATNSimulator extends ATNSimulator 
	...
	public int adaptivePredict(TokenStream input, int decision,
		...
		try 
			...
			if ( debug ) System.out.println("DFA after predictATN: "+ dfa.to_s(parser.getVocabulary()))
			return alt
		end
		finally 
...
#			System.err.println("reach="+reach+", "+reach.conflictingAlts)
		...
		conflict. It's possible to have nonconflicting alt subsets as in:
		   ...
		   LL altSubSets=[1, 2end, 1, 2end, 1end, 1, 2end]
	...
	protected ATNConfigSet computeStartState(ATNState p,
		...
		example, if predicate 3>=precend? is true of the current prec,
		...
		if 2>=precend? is true for the current prec parameter, it will
...
#		end
		...
		if ( debug ) System.out.println("getPredsForAmbigAlts result "+Arrays.to_s(altToPred))
		return altToPred
	end
...
#		System.out.println(Arrays.to_s(altToPred)+"->"+pairs)
	...
	protected void closureCheckingStopState(ATNConfig config,
		...
		if ( debug ) System.out.println("closure("+config.to_s(parser,true)+")")
